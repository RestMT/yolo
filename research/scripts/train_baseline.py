...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Послідовне донавчання базових моделей "
...
def train_model(
    ...
    if best_weights.exists():
        print()
        print(f"Пропущено {model_name}: результат уже існує.")
        print(f"Ваги: {best_weights}")
        return

    if last_weights.exists():
        raise RuntimeError(
            f"Знайдено незавершене навчання {model_name}: "
            f"{last_weights}\n"
    ...
    if run_directory.exists():
        raise RuntimeError(
            f"Каталог запуску вже існує, але best.pt і last.pt відсутні: "
            f"{run_directory}"
        )
    ...
    print()
    print(f"Навчання {model_name} завершено.")
...
def main() -> None:
    ...
    print()
    print("Навчання всіх вибраних моделей завершено.")
